...
$('.eqLogicAttr[data-l1key=configuration][data-l2key=model]').on('change', function () {
	var url;
	if($(this).value() === '') {
		url = '/plugins/Mattermost/plugin_info/Mattermost_icon.png';
	} else {
...
/*
 * Fonction pour l'ajout de commande, appellé automatiquement par plugin-Mattermost
 */
...
$('.eqLogicAction[data-action=sync_network]').on('click', function () {
	$.ajax({
		type: "POST",
		url: "plugins/Mattermost/core/ajax/Mattermost.ajax.php",
		data: {
			action: "sync_network"
		},
		dataType: 'json',
